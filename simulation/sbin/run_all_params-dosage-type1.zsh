...
echo "useage: run_all_params.zsh <repeat> <start> <output> <dosage>"
echo " |- repeat: number of repeats"
echo " |-  start: starting repeation number"
echo " |- output: output directory"
echo " |- effect: treatment effect"
echo "Generating commands for $repeat repeats starting from repeation number $start with treatment type-1 and dosage $dosage"

# policies=(null high-risk low-risk age-first)
# policies=(null high-risk low-risk)
# policies=(age-first high-risk-young-first)
  ...
  for scale_factor in $scale_factors; do
    for term_length in $term_lengths; do
      ...
      for policy in $policies; do
        echo "TR_LOW=$TR_LOW TR_HIGH=$TR_HIGH python -u run_policy.py \
