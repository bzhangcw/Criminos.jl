...
"""
function generate_fitting_Ω_bak(N, n, ℜ;
    ρₛ=rand(N),
    τₛ=rand(N),
    Σ₁=nothing,
    Σ₂=nothing,
    ...
    if style_mixin_monotonicity != 2
        ...
        # U-shaped
        @constraint(md, (_Bv * Σ₁ * T * ones(N) + _Bv * invT * Σ₂ * ones(N)) .== yv)
    end
    for (id, _Ψ) in enumerate(vec_Ψ)
    ...
    gₕ(τ) = _A * diagm(expt(τ)) * Σ₁ * diagm(expt(τ)) * ones(N) + _A * Σ₂ * ones(N)
    yₕ(τ) = Hₕ(τ) \ gₕ(τ)
...
function tuning(n, Σ, Σ₁, Σ₂, V; style_mixin_monotonicity=3, ρₛ=nothing, τₛ=nothing)
    if cc.style_correlation == :uppertriangular
        Σ₁ .= UpperTriangular(Σ₁)
        Σ₂ .= UpperTriangular(Σ₂)
