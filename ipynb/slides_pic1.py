...
def plot_slides_pic1(df):
    ...
    ax2 = ax1.twinx()
    rate_of_offending = [
        counts_R_full[idx] / counts_Np_full[idx]
        for idx in range(len(counts_R_full) - 1)
    ]
    ax2.plot(
        x[0:-1],
        rate_of_offending,
        marker="o",
        label=r"Rate of re-offending",
        linewidth=2.0,
    )
    ...
    return all_values, counts_N_full, counts_Np_full, counts_R_full
